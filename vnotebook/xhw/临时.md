...
select store_code,product_code from rbu_sxcp_edw_dev.fct_store_product_on_sale
"""
...
sql_pos_detail = """
select * from rbu_sxcp_edw_dev.fct_pos_detail where sale_date>='2018-08-01'
"""
...
sql_store_category = """
select * from rbu_sxcp_edw_dev.store_category where date>='2018-08-01'
"""
...
select * from rbu_sxcp_edw_dev.mid_sale_add_avg where sale_date>='2018-08-01'
"""
...
where a.reason='{0}' and c.shpcode is not null
""".format(hash('产品报废'))
...
def jiaoben(item):
    ...
    sql_sale_add = """
    select * from rbu_sxcp_edw_dev.fct_store_sale_add
    """
    ...
    spark.sql("use rbu_sxcp_edw_dev")
    drop_sql = """
    drop table rbu_sxcp_edw_dev.fct_store_sale_add_1
    """

    create_sql = """
    create table rbu_sxcp_edw_dev.fct_store_sale_add_2
    (store_code string,
    product_code string,
    check_date string,
    ...
    stored as  parquet
    location '/liufuya/edw.db/fct_store_sale_add_1'
    """
    start = time.time()
    #spark.sql(drop_sql)
    ...
    insert_sql = """
    insert into table rbu_sxcp_edw_dev.fct_store_sale_add_2 partition(month)
    (select
    store_code,
    product_code,
    check_date,
...
# sql_xhw = """
# select
# row_number() over(partition by store_code,product_code,check_date order by sale_add desc) date_id,
# store_code,
# product_code,
# check_date,
# sale_qty,
...
# where date_id=1
# """
# df_xhw = spark.sql(sql_xhw)
# df_xhw.createOrReplaceTempView("tmp_xhw")
```