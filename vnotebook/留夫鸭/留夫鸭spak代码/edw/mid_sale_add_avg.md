...
hc = HiveContext(sc)
get_current_timestamp = hc.udf.register("get_current_timestamp", edw_module.get_current_timestamp)
get_phone_number = hc.udf.register("get_phone_number", edw_module.get_number)
get_hash = hc.udf.register("get_hash", edw_module.get_hash)
get_date = hc.udf.register("get_date", edw_module.get_date)

print("starting..........")
print("平均销补数据的代码")
sql_mid_sale_add_sum = """
select * from rbu_sxcp_edw_dev.mid_sale_add_sum
"""
df_mid_sale_add_sum = sqlContext.sql(sql_mid_sale_add_sum)
df_mid_sale_add_sum.registerTempTable("mid_sale_add_sum_spark")
within_a_year = hc.udf.register("within_a_year", edw_module.within_a_year)
...
def jiaoben(item):
    ...
    where Boolean(within_a_year('{0}',sale_date))
    """.format(item)
    df_tmp1_1 = sqlContext.sql(tmp1_1_sql)
    df_tmp1_1.registerTempTable("tmp1_1_spark")
    ...
    df_tmp1 = sqlContext.sql(tmp1_sql)
    df_tmp1.registerTempTable("tmp1_spark")
    ...
    """
    df_tmp2_1 = sqlContext.sql(tmp2_1_sql)
    df_tmp2_1.registerTempTable("tmp2_1_spark")
    ...
    from tmp2_1_spark
    """
    df_tmp2 = sqlContext.sql(tmp2_sql)
    df_tmp2.registerTempTable("tmp2_spark")
    ...
    """
    df_tmp3 = sqlContext.sql(tmp3_sql)
    df_tmp3.registerTempTable("tmp3_spark")
    ...
    from tmp3_spark
    """
    df_tmp4 = sqlContext.sql(tmp4_sql)
    df_tmp4.registerTempTable("tmp4_spark")
    ...
    from tmp4_spark 
    """
    df_tmp5_1 = sqlContext.sql(tmp5_1_sql)
    df_tmp5_1.registerTempTable("tmp5_1_spark")
    ...
    where date_id<=30
    """
    df_tmp5 = sqlContext.sql(tmp5_sql)
    df_tmp5.registerTempTable("tmp5_spark")
    ...
    date_case
    """
    df_test = sqlContext.sql(sql_test)
    df_test.registerTempTable("tmp_test")

    sql_fct_daily_stock = """
    select * from  rbu_sxcp_edw_dev.fct_daily_stock
    """

    df_fct_daily_stock = sqlContext.sql(sql_fct_daily_stock)
    df_fct_daily_stock.registerTempTable("tmp_fct_daily_stock")
    hc.sql("use rbu_sxcp_edw_dev")
    insert_sql = """
    insert into table mid_sale_add_avg
    (
    select 
    store_code,
    product_code,
...
for i in range(0, 31):
    min_day = datetime.datetime.strptime('2017-12-31', "%Y-%m-%d") + datetime.timedelta(days=-i)
