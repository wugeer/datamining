...
spark = SparkSession.builder.master("yarn").appName("turnover_feat").enableHiveSupport().getOrCreate()

print("starting..........")

sql_fct_store_product_on_sale = """
select * from rbu_sxcp_edw_dev.fct_store_product_on_sale_1
"""
...
sql_dim_store = """
select * from rbu_sxcp_edw_dev.dim_store_1
"""
df_dim_store = spark.sql(sql_dim_store)
df_dim_store.createOrReplaceTempView("tmp_dim_store")

tmp1_sql = """
select distinct store_code from tmp_fct_store_product_on_sale
"""
...
sql_store_category = """
select * from rbu_sxcp_ods_dev.store_category
"""
...
select * from rbu_sxcp_edw_dev.dim_store_product_1
"""
...
left join tmp_fct_store_sale_add t10 on t9.store_code=t10.store_code and t9.date=t10.check_date
left join tmp_dim_store_product t11 on t10.store_code=t11.store_code and t10.product_code=t11.product_code
where t9.date>='2017-07-01' 
group by t8.store_code,t9.date,t9.date_case
"""
df_tmp2 = spark.sql(tmp2_sql)
df_tmp2.createOrReplaceTempView("tmp2_spark")

sql_fct_pos_detail = """
select * from rbu_sxcp_edw_dev.fct_pos_detail_1
"""
...
left join tmp_fct_pos_detail t2 on t1.store_code=t2.store_code and t1.date=t2.sale_date
...
def jiaoben(item1, item2):
    ...
    from tmp1_spark t13
    left join tmp_store_category t14 on t13.store_code=t14.store_code and t14.date='{0}'
    ...
    # df_tmp39.createOrReplaceTempView("tmp39_spark")
...
for i in range(0, 14):
    print("营业额特征数据的代码")
    min_day = datetime.datetime.strptime('2018-12-01', "%Y-%m-%d") + datetime.timedelta(days=i)
