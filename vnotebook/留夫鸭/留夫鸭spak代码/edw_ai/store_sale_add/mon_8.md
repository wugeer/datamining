...
get_date = spark.udf.register("get_date", get_date)


def get_valid_day(start, end, step):
    t = datetime.datetime.strptime(start[:10], "%Y-%m-%d") + datetime.timedelta(days=-step)
    return str(t)[:10] > end[:10]


get_valid_day = spark.udf.register("get_valid_day", get_valid_day)


def check_with_day(start, item):
    min_day = datetime.datetime.strptime(start, "%Y-%m-%d") + datetime.timedelta(days=-7)
    item = datetime.datetime.strptime(item, "%Y-%m-%d")
    return min_day >= item


check_with_day = spark.udf.register("check_with_day", check_with_day)


def get_yesterday(item):
    yes = datetime.datetime.strptime(item, "%Y-%m-%d") + datetime.timedelta(days=-1)
    return str(yes)[:10]


get_yesterday = spark.udf.register("get_yesterday", get_yesterday)
print("starting..........")

sql_fct_store_product_on_sale = """
select store_code,product_code from rbu_sxcp_edw_dev.fct_store_product_on_sale_1
"""
...
mon = ['2018-08-01', '2018-09-01']

sql_pos_detail = """
select * from rbu_sxcp_edw_dev.fct_pos_detail_1 where sale_date>='{0}' and sale_date<'{1}'
""".format(mon[0], mon[1])
...
sql_store_category = """
select shop_code,date,case from rbu_sxcp_ods_dev.store_category where date>='{0}' and date<'{1}'
""".format(mon[0], mon[1])
...
select * from rbu_sxcp_edw_ai_dev.sale_add_avg where sale_date>='{0}' and sale_date<'{1}'
""".format(mon[0], mon[1])
...
sql_fct_daily_stock = """
select store_code,product_code,to_date(check_date) check_date,qty from rbu_sxcp_edw_dev.fct_daily_stock_1 where to_date(check_date)>='{0}' and to_date(check_date)<'{1}'
""".format(mon[0], mon[1])
...
def jiaoben(item):
    ...
    before_date
    from tmp8_spark a
    where Boolean(check_with_day(check_date,before_date))
    and not exists
    (
        select 1 from tmp_ods_product_bill_shop b
        where a.store_code=b.shop_code
            and a.product_code=b.product_code
            and b.bill_date>=get_yesterday('{0}')
            and b.bill_date<='{0}'
    )
    """.format(item)
    start = time.time()
...
for i in range(0, 31):
    ...
    min_day = datetime.datetime.strptime('2018-08-01', "%Y-%m-%d") + datetime.timedelta(days=i)
