...
select store_code,product_code from rbu_sxcp_edw_dev.fct_store_product_on_sale_history
where sale_date='2019-01-02'
"""
...
sql_store_category = """
select shop_code store_code, date,case date_case from  rbu_sxcp_ods_dev.store_category
"""
...
def generate_series(start, end, step):
    res = [[start], ]
    while res[-1][0] < end:
        res.append([str(datetime.datetime.strptime(res[-1][0], "%Y-%m-%d") + datetime.timedelta(days=step))[:10]])
    return res


def jiaoben(item):
    time_list = generate_series('2016-07-01', '2019-01-03', 1)
