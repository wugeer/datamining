...
days = [27, 31, 30, 14]
first_dates = ['2018-09-04', '2018-10-01', '2018-11-01', '2018-12-01']
...
sql_dim_store = """
select * from rbu_sxcp_edw_dev.dim_store  where store_code in ('st_code_0585', 'st_code_0206', 'st_code_0648') 
"""
df_dim_store = spark.sql(sql_dim_store)
df_dim_store.createOrReplaceTempView("tmp_dim_store")

sql_fct_store_sale_add = """
select row_number() over(partition by store_code,product_code,check_date order by etl_time desc) date_id,
store_code,product_code,check_date,sale_total_qty,etl_time 
from rbu_sxcp_edw_ai_dev.store_sale_add
"""
df_fct_store_sale_add_1 = spark.sql(sql_fct_store_sale_add)
df_fct_store_sale_add_1.createOrReplaceTempView("tmp_fct_store_sale_add_1")

sql_2 = """
select store_code,product_code,check_date,sale_total_qty from tmp_fct_store_sale_add_1 where date_id=1
"""
df_fct_store_sale_add = spark.sql(sql_2)
df_fct_store_sale_add.createOrReplaceTempView("tmp_fct_store_sale_add")

sql_dim_store_product = """
select * from rbu_sxcp_edw_dev.dim_store_product where store_code in ('st_code_0585', 'st_code_0206', 'st_code_0648') 
"""
...
sql_fct_pos_detail = """
select * from rbu_sxcp_edw_dev.fct_pos_detail where store_code in ('st_code_0585', 'st_code_0206', 'st_code_0648') 
"""
...
def mon_loop(index, mon):
    sql_fct_store_product_on_sale = """
    select store_code,sale_date from rbu_sxcp_edw_dev.fct_store_product_on_sale_history
    where sale_date>='{0}' and sale_date<'{1}' and store_code in ('st_code_0585', 'st_code_0206', 'st_code_0648') 
    """.format(mon[0], mon[1])
    ...
    for i in range(0, days[index]):
        min_day = datetime.datetime.strptime(first_dates[index], "%Y-%m-%d") + datetime.timedelta(days=i)
        one_year_age = datetime.datetime.strptime(str(min_day)[:10], "%Y-%m-%d") + datetime.timedelta(days=-365)
        print("%s 营业额特征数据的代码" % str(min_day)[:10])
        item1 = str(one_year_age)[:10]
        item2 = str(min_day)[:10]
        tmp1_sql = """
        ...
        spark.catalog.cacheTable("tmp1_spark")
        ...
        # print("tmp_mid_turnover_feat_sql_spark全部")
        # spa# rk.sql("select * from tmp_mid_turnover_feat_sql_spark where store_code='st_code_0648' and sale_date='2018-09-07'").show()
        # print("tmp# _mid_turnover_feat_sql_spark 为null部分")
        # spark.sql("select * from tmp_mid_turnover_feat_sql_spark where store_code='st_code_0648' and sale_date='2018-09-07' and turnover is null").show()
        ...
        #print("tmp_33")
        #spark.sql("select * from tmp33_spark ").show()
        tmp34_sql = """
        select 
        store_code,
        sale_date,
        ...
        df_tmp35_1.drop()
        print("recent_3")
        #spark.sql("select * from tmp36_spark").show()
        ...
        print("recent_7")
        #spark.sql("select * from tmp37_spark").show()
        ...
        insert_sql = """
        insert into table rbu_sxcp_edw_ai_dev.turnover_feat_test partition(month)
        (select 
        t8.store_code,
        t8.sale_date,
        t8.date_case,
        ...
        from tmp38_spark t8
        left join tmp32_spark t9 on t8.store_code=t9.store_code 
        left join tmp36_spark t10 on t8.store_code=t10.store_code
        left join tmp37_spark t11 on t8.store_code=t11.store_code
        left join tmp_mid_turnover_feat_sql_spark t12 on t8.store_code=t12.store_code and t8.sale_date=t12.sale_date
        where t8.sale_date='{0}' and t10.recent_3 is not null and t10.recent_3>=0 and t11.recent_7 is not null and t11.recent_7>=0)
        DISTRIBUTE BY rand()
        """.format(item2)
        ...
        df_tmp_mid_turnover_feat_sql.drop()
        print("删除临时表成功")


for i in range(0, 4):
    mon = [['2018-09-01', '2018-10-01'], ['2018-10-01', '2018-11-01'], ['2018-11-01', '2018-12-01'], ['2018-12-01', '2018-12-15']]
    mon_loop(i, mon[i])
