...
def jiaoben(item1, item2):
    ...
    #print("测试一下date_case")
    #spark.sql("select store_code,sale_date,date_case from tmp2_spark where sale_date='2018-09-07' and store_code='st_code_0659'").show()
    ...
    #print("测试一下tmp_mid_turnover_feat_sql_spark")
    #spark.sql("select store_code,sale_date,date_case from tmp_mid_turnover_feat_sql_spark where sale_date='2018-09-07' and store_code='st_code_0659'").show()
    ...
    tmp33_1_sql = """
    select distinct store_code, date_case from rbu_sxcp_edw_ai_dev.turnover_feat where sale_date=date_sub('{0}',1)
    """.format(item2)
    ...
    tmp33_sql = """
    select 
    row_number() over(partition by a.store_code,b.date_case order by sale_date desc) date_id,
    a.store_code,
    sale_date,
    b.date_case,
    turnover
    from rbu_sxcp_edw_ai_dev.turnover_feat a 
    left join tmp33_1 b on a.store_code=b.store_code
    where sale_date<'{0}' and turnover is not null and b.date_case is not null
    """.format(item2)
    df_tmp33 = spark.sql(tmp33_sql)
    df_tmp33.createOrReplaceTempView("tmp33_spark")
    #print("tmp33")
    #df_tmp33.show()
    #spark.sql("select store_code,sale_date,date_case from tmp33_spark where sale_date='2018-09-07' and store_code='st_code_0659'").show()
    tmp34_sql = """
    select 
    store_code,
    sale_date,
    ...
    """.format(item2)
    # where t10.recent_3 is not null and t10.recent_3>=0 and t11.recent_7 is not null and t11.recent_7>=0
